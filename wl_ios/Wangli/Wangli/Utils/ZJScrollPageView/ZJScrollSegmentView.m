...
- (void)titleLabelOnClick:(UITapGestureRecognizer *)tapGes {
    ...
    [self adjustUIWhenBtnOnClickWithAnimate:true taped:YES enBlock:YES];
}
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped enBlock:(BOOL)enBlock {
...
//    if (self.titleBtnOnClick) {
    // eidt by yeqiang
    if (self.titleBtnOnClick && enBlock) {
...
- (void)setSelectedIndex:(NSInteger)index animated:(BOOL)animated enBlock:(BOOL)enBlock {
    ...
    _currentIndex = index;
    [self adjustUIWhenBtnOnClickWithAnimate:animated taped:NO enBlock:enBlock];
}
...
- (void)reloadTitlesWithNewTitles:(NSArray *)titles {
    ...
    [self setupSubviews];
    [self setupUI];
    [self setSelectedIndex:0 animated:YES enBlock:YES];
...
- (UIView *)coverLayer {
    ...
    if (_coverLayer == nil) {
        ...
        coverView.layer.borderColor = self.segmentStyle.selectedTitleColor.CGColor;
...
//    __block NSInteger index = _currentIndex;
//    [self.titleWidths enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//        CGFloat width = [obj floatValue];
//        if (total > offsetX) {
//            index = idx;
//            *stop = YES;
//        } else {
//            total = total + width + 25;
//        }
//    }];
//
//    _currentIndex = index;
//
//    [self adjustUIWithProgress:1.0 oldIndex:oldIndex currentIndex:_currentIndex];
//}
