...
// 判断是否是iPhone X
#define isiPhoneX ([UIScreen instancesRespondToSelector:@selector(currentMode)] ?(CGSizeEqualToSize(CGSizeMake(375, 812),[UIScreen mainScreen].bounds.size) || CGSizeEqualToSize(CGSizeMake(414, 896),[UIScreen mainScreen].bounds.size)): NO)
// home indicator
...
-(void)setupUI {
    ...
    self.doneButtonColor = COLOR_C1;// RGB(247, 133, 51);
    ...
    [self.cancelBtn setTitleColor:COLOR_B1 forState:UIControlStateNormal];
    self.cancelBtn.titleLabel.font = FONT_F15;
    [self.cancelBtn setBackgroundColor:COLOR_F2F3F5];
    
    [self.doneBtn setTitleColor:COLOR_1893D5 forState:UIControlStateNormal];
    self.doneBtn.titleLabel.font = FONT_F15;
    [self.doneBtn setBackgroundColor:COLOR_F2F3F5];
...
-(void)dismiss {
    [UIView animateWithDuration:.3 animations:^{
        self.bottomConstraint.constant =  -SCREEN_HEIGHT;//-self.height;
        self.backgroundColor = RGBA(0, 0, 0, 0);
        [self layoutIfNeeded];
...
- (void)setBtnCancelTitle:(NSString *)btnCancelTitle {
    [_cancelBtn setTitle:btnCancelTitle forState:UIControlStateNormal];
}
