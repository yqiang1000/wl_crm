...
//    _pointlineArr = nil;
    ...
    for (int i= 0; i < self.viewArr.count; i++) {
        ...
        if (i < self.textView.text.length) {
            [self changeViewLayerIndex:i pointHidden:YES];
            label.text = [ self.textView.text substringWithRange:NSMakeRange(i, 1)];
            
        }else{
            [self changeViewLayerIndex:i pointHidden:i== self.textView.text.length?NO:YES];
            if (! self.textView.text&& self.textView.text.length==0) {//textView的text为空的时候
