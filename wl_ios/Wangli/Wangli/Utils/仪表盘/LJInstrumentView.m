...
@property (nonatomic, strong) ArrowLayer *layermin;
...
- (void)drawProgressCicrleWithfillColor:(UIColor*)fillColor strokeColor:(UIColor*)strokeColor{
    ...
    [self.layermin countFrom:-M_PI*5/4 to:-M_PI*5/4 + 0.01 * _speedValue * (self.endAngle - self.startAngle) withDuration:1];
}
...
- (void)setLayer {
    self.layermin = [ArrowLayer layer];
    self.layermin.frame = CGRectMake(self.bounds.size.width/2-21.0/2,self.bounds.size.height/2-21.0/2, 62,21);
    self.layermin.position = CGPointMake(self.center.x, self.center.y);
    [self.layer addSublayer:self.layermin];
    [self.layermin setNeedsDisplay];
    self.layermin.anchorPoint = CGPointMake(0.2, 0.5);
    self.layermin.delegate = self.layermin;
}
