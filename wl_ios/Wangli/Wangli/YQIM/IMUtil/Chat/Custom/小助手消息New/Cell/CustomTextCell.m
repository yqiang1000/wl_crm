...
- (UIView *)setupUI {
    ...
    [self.baseView addSubview:self.labMore];
    ...
    [self.labMore mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)fillWithData:(NewHelpeMessageCellData *)data {
    ...
    NSInteger tableItemCount = self.customData.dataMo.Cells.count;
    // 如果没有处理过，才去查询数据库
    ...
    self.labMore.attributedText = [YQNewChatUtils moreContent:self.customData.dataMo.MoreInfo];
    [self.labMore addLinkWithType:MLLinkTypeURL value:nil range:NSMakeRange(0, self.customData.dataMo.MoreInfo.content.length)];
    [self.labMore setDidLongPressLinkBlock:^(MLLink *link, NSString *linkText, MLLinkLabel *label) {
        NSLog(@"click 查看更多 ---- %@", linkText);
        NSMutableDictionary *userInfo = [NSMutableDictionary new];
        [userInfo setObject:STRING(linkText) forKey:@"linkText"];
        [userInfo setObject:STRING(self.customData.dataMo.MoreInfo.url) forKey:@"url"];
...
- (MLLinkLabel *)labMore {
    if (!_labMore) {
        _labMore = [[MLLinkLabel alloc] init];
        _labMore.textColor = COLOR_LINK;
        _labMore.font = FONT_F15;
        _labMore.numberOfLines = 0;
    }
    return _labMore;
}
