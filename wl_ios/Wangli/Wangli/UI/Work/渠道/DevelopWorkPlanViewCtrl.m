...
@interface DevelopWorkPlanViewCtrl () <UITableViewDelegate, UITableViewDataSource, ListSelectViewCtrlDelegate, MyTextViewCellDelegate, DefaultInputCellDelegate, MemberSelectViewCtrlDelegate, JYUserMoSelectViewCtrlDelegate, MySwitchCellDelegate>

@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) NSMutableArray *arrData;      // 表单模型数组
@property (nonatomic, strong) NSMutableArray *arrVisits;
@property (nonatomic, strong) NSMutableArray *arrSigns;
...
@implementation DevelopWorkPlanViewCtrl
...
- (void)getDetailModel {
    [Utils showHUDWithStatus:nil];
    [[JYUserApi sharedInstance] getChannelDevelopDetail:self.model.id param:nil success:^(id responseObject) {
        [Utils dismissHUD];
        NSError *error = nil;
        self.model = [[ChannelDevelopmentMo alloc] initWithDictionary:responseObject error:&error];
...
- (void)configRowMos {
    ...
    rowMo1.editAble = NO;
    rowMo1.key = @"developmentProject";
    if (self.model) {
        rowMo1.strValue = [NSString stringWithFormat:@"%ld", (long)self.model.developmentProject];
        rowMo1.m_obj = [NSString stringWithFormat:@"%ld", (long)self.model.developmentProject];
        rowMo1.value = [NSString stringWithFormat:@"%ld", (long)self.model.developmentProject];
    }
    ...
    rowMo2.editAble = NO;
    rowMo2.key = @"accumulateVisit";
    if (self.model) {
        rowMo2.strValue = [NSString stringWithFormat:@"%ld", (long)self.model.accumulateVisit];
        rowMo2.m_obj = [NSString stringWithFormat:@"%ld", (long)self.model.accumulateVisit];
        rowMo2.value = [NSString stringWithFormat:@"%ld", (long)self.model.accumulateVisit];
    }
    ...
    rowMo3.editAble = NO;
    rowMo3.key = @"visitIntentional";
    if (self.model.visitIntentional > 0) {
        rowMo3.strValue = [NSString stringWithFormat:@"%ld", (long)self.model.visitIntentional];
        rowMo3.m_obj = [NSString stringWithFormat:@"%ld", (long)self.model.visitIntentional];
        rowMo3.value = [NSString stringWithFormat:@"%ld", (long)self.model.visitIntentional];
    }
    ...
    rowMo4.editAble = NO;
    rowMo4.key = @"finishVisit";
    if (self.model.finishVisit) {
        rowMo4.strValue = [NSString stringWithFormat:@"%ld", (long)self.model.finishVisit];
        rowMo4.m_obj = [NSString stringWithFormat:@"%ld", (long)self.model.finishVisit];
        rowMo4.value = [NSString stringWithFormat:@"%ld", (long)self.model.finishVisit];
    }
    ...
    rowMo7.leftContent = @"签署意向客户";
    rowMo7.inputType = K_SHORT_TEXT;
    rowMo7.rightContent = @"自动带出";
    rowMo7.editAble = NO;
    rowMo7.key = @"signIntention";
    if (self.model.signIntention > 0) {
        rowMo7.strValue = [NSString stringWithFormat:@"%ld", (long)self.model.signIntention];
    }
    [self.arrData addObject:rowMo7];
    
    CommonRowMo *rowMo8 = [[CommonRowMo alloc] init];
    rowMo8.rowType = K_INPUT_TEXT;
    rowMo8.leftContent = @"实际签署";
    rowMo8.inputType = K_SHORT_TEXT;
    rowMo8.rightContent = @"自动带出";
    rowMo8.editAble = NO;
    rowMo8.key = @"actualSign";
    if (self.model.actualSign > 0) {
        rowMo8.strValue = [NSString stringWithFormat:@"%ld", (long)self.model.actualSign];
    }
...
//    if (self.model.finish) [self.arrData addObject:self.attRowMo];
    ...
    [_arrVisits removeAllObjects];
    _arrVisits = nil;
    [_arrSigns removeAllObjects];
    _arrSigns = nil;
    for (int i = 0; i < self.model.visitIntentions.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.visitIntentions[i];
        VisitIntentionMo *tmpMo = [[VisitIntentionMo alloc] initWithDictionary:dic error:&error];
        [self.arrVisits addObject:tmpMo];
    }
    for (int i = 0; i < self.model.signIntentions.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.signIntentions[i];
        SignIntentionMo *tmpMo = [[SignIntentionMo alloc] initWithDictionary:dic error:&error];
        [self.arrSigns addObject:tmpMo];
    }
...
- (void)getProvinceList {
    ...
    [[JYUserApi sharedInstance] getWorkPlanTargetByOperatorType:@"2" param:@{@"id":@(userMo.id),@"oaCode":STRING(userMo.oaCode)} success:^(id responseObject) {
...
- (void)reSelectedUser:(BOOL)reSelectedUser reSelectedProvince:(BOOL)reSelectedProvince {
    ...
    for (int i = 0; i < self.arrData.count; i++) {
        ...
        if ([tmpRowMo.key isEqualToString:@"operator"]) {
            ...
            provinceIndex = i;
            count++;
        } else if ([tmpRowMo.key isEqualToString:@"developmentProject"]) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 1) return self.arrVisits.count;
    if (section == 2) return self.arrSigns.count;
    return 0;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    if (section == 1 && !self.afterDate) {
        return [self setupFooterViewWithIdentifier:@"section1" selector:@selector(createVisit:)];
    } else if (section == 2 && !self.afterDate) {
        return [self setupFooterViewWithIdentifier:@"section2" selector:@selector(createSign:)];
    }
    return [UIView new];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        if (indexPath.section == 1) {
            VisitIntentionMo *tmpMo = self.arrVisits[indexPath.row];
            cell.labTitle.text = tmpMo.member;
        } else if (indexPath.section == 2) {
            SignIntentionMo *tmpMo = self.arrSigns[indexPath.row];
            cell.labTitle.text = tmpMo.member;
        }
        return cell;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        [self hidenKeyboard];
        VisitIntentionMo *itemMo = self.arrVisits[indexPath.row];
        ...
        vc.updateSuccess = ^(id obj) {
            ...
            [strongself updateVisitMemberCount];
        };
        [self.navigationController pushViewController:vc animated:YES];
    }
    else if (indexPath.section == 2) {
        [self hidenKeyboard];
        SignIntentionMo *itemMo = self.arrSigns[indexPath.row];
        ...
        vc.updateSuccess = ^(id obj) {
            __strong typeof(self) strongself = weakself;
            [strongself updateSignMemberCount];
        };
        [self.navigationController pushViewController:vc animated:YES];
    }
}
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIAlertAction *action2 = [UIAlertAction actionWithTitle:GET_LANGUAGE_KEY(@"确定") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if (indexPath.section == 1) {
            [self.arrVisits removeObjectAtIndex:indexPath.row];
            [self updateVisitMemberCount];
        } else if (indexPath.section == 2) {
            [self.arrSigns removeObjectAtIndex:indexPath.row];
            [self updateSignMemberCount];
        }
    }];
...
- (void)createVisit:(UIButton *)sender {
    ...
    vc.updateSuccess = ^(VisitIntentionMo *tmpMo) {
        __strong typeof(self) strongself = weakself;
        [strongself.arrVisits addObject:tmpMo];
...
- (void)createSign:(UIButton *)sender {
    ...
    vc.updateSuccess = ^(SignIntentionMo *tmpMo) {
        __strong typeof(self) strongself = weakself;
        [strongself.arrSigns addObject:tmpMo];
        [strongself.tableView reloadData];
        [strongself updateSignMemberCount];
    };
    [self.navigationController pushViewController:vc animated:YES];
...
- (void)updateVisitMemberCount {
    for (CommonRowMo *rowMo in self.arrData) {
        if ([rowMo.key isEqualToString:@"visitIntentional"]) {
            rowMo.strValue = [NSString stringWithFormat:@"%lu", (long)self.arrVisits.count];
            rowMo.value = [NSString stringWithFormat:@"%lu", (long)self.arrVisits.count];
        }
        else if ([rowMo.key isEqualToString:@"finishVisit"]) {
            NSInteger count = 0;
            for (int i = 0; i < self.arrVisits.count; i++) {
                VisitIntentionMo *tmpMo = self.arrVisits[i];
...
- (void)updateSignMemberCount {
    for (CommonRowMo *rowMo in self.arrData) {
        if ([rowMo.key isEqualToString:@"signIntention"]) {
            rowMo.strValue = [NSString stringWithFormat:@"%lu", (long)self.arrSigns.count];
            rowMo.value = [NSString stringWithFormat:@"%lu", (long)self.arrSigns.count];
        }
        else if ([rowMo.key isEqualToString:@"actualSign"]) {
            NSInteger count = 0;
            for (int i = 0; i < self.arrSigns.count; i++) {
                SignIntentionMo *tmpMo = self.arrSigns[i];
                if (tmpMo.sign) count++;
            }
            rowMo.strValue = [NSString stringWithFormat:@"%ld", (long)count];
            rowMo.value = [NSString stringWithFormat:@"%ld", (long)count];
...
- (void)clickRightButton:(UIButton *)sender {
    ...
    if (self.arrVisits.count < 3) {
        [Utils showToastMessage:@"拜访意向客户不能少于3个"];
        return;
    }
...
- (void)dealWithParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    for (VisitIntentionMo *tmpMo in self.arrVisits) {
    ...
    [params setObject:visitArr forKey:@"visitIntentions"];
    
    // 签署客户
    NSMutableArray *signArr = [NSMutableArray new];
    for (SignIntentionMo *tmpMo in self.arrSigns) {
        [signArr addObject:@{@"member": STRING(tmpMo.member),
                              @"sign":@(tmpMo.sign)}];
    }
    [params setObject:signArr forKey:@"signIntentions"];
...
- (void)createParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    [[JYUserApi sharedInstance] createDynmicFormDynamicId:@"channel-development" param:params success:^(id responseObject) {
        [Utils dismissHUD];
...
- (void)updateParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    [[JYUserApi sharedInstance] updateDynmicFormDynamicId:@"channel-development" param:params success:^(id responseObject) {
        [Utils dismissHUD];
...
- (NSMutableArray *)arrVisits {
    if (!_arrVisits) _arrVisits = [NSMutableArray new];
    return _arrVisits;
}

- (NSMutableArray *)arrSigns {
    if (!_arrSigns) _arrSigns = [NSMutableArray new];
    return _arrSigns;
}
