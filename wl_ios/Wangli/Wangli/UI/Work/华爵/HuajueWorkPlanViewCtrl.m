...
@interface HuajueWorkPlanViewCtrl () <UITableViewDelegate, UITableViewDataSource, ListSelectViewCtrlDelegate, MyTextViewCellDelegate, DefaultInputCellDelegate, MemberSelectViewCtrlDelegate, JYUserMoSelectViewCtrlDelegate, MySwitchCellDelegate>
...
@implementation HuajueWorkPlanViewCtrl
...
- (void)getYesterDayDataWorkType {
    ...
    [[JYUserApi sharedInstance] getYesterdayWorkDataWorkType:WORKTYPE_HUA_JUE param:@{@"loginOperator":@(TheUser.userMo.id), @"yesterday":yesterdayStr} success:^(id responseObject) {
        [Utils dismissHUD];
        NSError *error = nil;
        self.model = [[HuajueMo alloc] initWithDictionary:responseObject error:&error];
        self.createDate = YES;
...
- (void)getDetailModel {
    [Utils showHUDWithStatus:nil];
    [[JYUserApi sharedInstance] getHuajueDetail:self.model.id param:nil success:^(id responseObject) {
        [Utils dismissHUD];
        NSError *error = nil;
        self.model = [[HuajueMo alloc] initWithDictionary:responseObject error:&error];
...
//    NSDate *nextDay = [NSDate dateWithTimeInterval:24*60*60 sinceDate:today];//后一天
    ...
    _arrUnFulFilled = nil;
    for (int i = 0; i < self.model.huaJueItems.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.huaJueItems[i];
    ...
    for (int i = 0; i < self.model.hjReportingProjects.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.hjReportingProjects[i];
        ReportingProjectMo *tmpMo = [[ReportingProjectMo alloc] initWithDictionary:dic error:&error];
        [self.arrReporting addObject:tmpMo];
    }
    for (int i = 0; i < self.model.hjUnfulfilledProjects.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.hjUnfulfilledProjects[i];
...
- (void)loadAttachement:(NengChengItemMo *)item {
    [[JYUserApi sharedInstance] getHuajueItemDetail:item.id param:nil success:^(id responseObject) {
        NSError *error = nil;
...
- (void)getMonthTotalSum {
    ...
    [[JYUserApi sharedInstance] getWorkSumAcutalShipmentType:@"hua-jue" province:STRING(provinceRowMo.strValue) param:@{@"id":@(userMo.id)} success:^(id responseObject) {
...
- (void)dealWithParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    [params setObject:memberArr forKey:@"huaJueItems"];
    ...
    [params setObject:reportArr forKey:@"hjReportingProjects"];
    ...
    [params setObject:unfulArr forKey:@"hjUnfulfilledProjects"];
...
- (void)createParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    [[JYUserApi sharedInstance] createDynmicFormDynamicId:@"hua-jue" param:params success:^(id responseObject) {
        [Utils dismissHUD];
...
- (void)updateParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    [[JYUserApi sharedInstance] updateDynmicFormDynamicId:@"hua-jue" param:params success:^(id responseObject) {
        [Utils dismissHUD];
