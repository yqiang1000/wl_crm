...
@interface MarketWorkPlanViewCtrl () <UITableViewDelegate, UITableViewDataSource, ListSelectViewCtrlDelegate, MyTextViewCellDelegate, DefaultInputCellDelegate, MemberSelectViewCtrlDelegate, JYUserMoSelectViewCtrlDelegate, MySwitchCellDelegate>
...
@implementation MarketWorkPlanViewCtrl
...
- (void)getYesterDayDataWorkType {
    ...
    [[JYUserApi sharedInstance] getYesterdayWorkDataWorkType:WORKTYPE_MARKET_ENGINEERING param:@{@"loginOperator":@(TheUser.userMo.id), @"yesterday":yesterdayStr} success:^(id responseObject) {
        [Utils dismissHUD];
        NSError *error = nil;
        self.model = [[MarketEngineeringMo alloc] initWithDictionary:responseObject error:&error];
        self.createDate = YES;
...
- (void)getDetailModel {
    [Utils showHUDWithStatus:nil];
    [[JYUserApi sharedInstance] getMarketEngineeringDetail:self.model.id param:nil success:^(id responseObject) {
        [Utils dismissHUD];
        NSError *error = nil;
        self.model = [[MarketEngineeringMo alloc] initWithDictionary:responseObject error:&error];
...
//    NSDate *nextDay = [NSDate dateWithTimeInterval:24*60*60 sinceDate:today];//后一天
    ...
    rowMo14.leftContent = @"报备工程跟进";
    rowMo14.inputType = K_SHORT_TEXT;
    rowMo14.rightContent = @"请输入";
    rowMo14.editAble = self.beforeDate || self.createDate;
    rowMo14.nullAble = YES;
    rowMo14.keyBoardType = K_INTEGER;
    rowMo14.key = @"followUpReportingProject";
    if (self.model) {
        rowMo14.strValue = [NSString stringWithFormat:@"%ld", (long)self.model.followUpReportingProject];
        rowMo14.m_obj = [NSString stringWithFormat:@"%ld", (long)self.model.followUpReportingProject];
        rowMo14.value = [NSString stringWithFormat:@"%ld", (long)self.model.followUpReportingProject];
    }
    [self.arrData addObject:rowMo14];
    
    CommonRowMo *rowMo15 = [[CommonRowMo alloc] init];
    rowMo15.rowType = K_INPUT_TEXT;
    rowMo15.leftContent = @"未履行工程跟进";
    rowMo15.inputType = K_SHORT_TEXT;
    rowMo15.rightContent = @"请输入";
    rowMo15.editAble = self.beforeDate || self.createDate;
    rowMo15.nullAble = YES;
    rowMo15.keyBoardType = K_INTEGER;
    rowMo15.key = @"followUpUnfulfilledProject";
    if (self.model) {
        rowMo15.strValue = [NSString stringWithFormat:@"%ld", (long)self.model.followUpUnfulfilledProject];
        rowMo15.m_obj = [NSString stringWithFormat:@"%ld", (long)self.model.followUpUnfulfilledProject];
        rowMo15.value = [NSString stringWithFormat:@"%ld", (long)self.model.followUpUnfulfilledProject];
    }
    ...
    _arrUnFulFilled = nil;
    for (int i = 0; i < self.model.marketEngineeringItems.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.marketEngineeringItems[i];
        MarketEngineeringItemMo *tmpMo = [[MarketEngineeringItemMo alloc] initWithDictionary:dic error:&error];
        [self.arrMarketItem addObject:tmpMo];
    }
    for (int i = 0; i < self.model.reportingProjects.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.reportingProjects[i];
        ReportingProjectMo *tmpMo = [[ReportingProjectMo alloc] initWithDictionary:dic error:&error];
        [self.arrReporting addObject:tmpMo];
    }
    for (int i = 0; i < self.model.unfulfilledProjects.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.unfulfilledProjects[i];
...
- (void)getMonthTotalSum {
    ...
    [[JYUserApi sharedInstance] getWorkSumAcutalShipmentType:@"market-engineering" province:STRING(provinceRowMo.strValue) param:@{@"id":@(userMo.id)} success:^(id responseObject) {
...
#pragma mark - MyCommonHeaderViewDelegate

//- (void)myCommonHeaderView:(MyCommonHeaderView *)myCommonHeaderView didSelected:(NSInteger)section {
//    if (section == 1) {
...
//                MarketEngineeringItemMo *tmpMo = [[MarketEngineeringItemMo alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        if (indexPath.section == 1) {
            MarketEngineeringItemMo *tmpMo = self.arrMarketItem[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        MarketEngineeringItemMo *itemMo = self.arrMarketItem[indexPath.row];
        MarketEngineerItemViewCtrl *vc = [[MarketEngineerItemViewCtrl alloc] init];
...
- (void)createMarket:(UIButton *)sender {
    ...
    vc.afterDate = self.afterDate;
    __weak typeof(self) weakself = self;
    vc.updateSuccess = ^(MarketEngineeringItemMo *tmpMo) {
        __strong typeof(self) strongself = weakself;
    ...
    for (CommonRowMo *rowMo in self.arrData) {
        if ([rowMo.key isEqualToString:@"expectVisit"]) {
            ...
            for (int i = 0; i < self.arrMarketItem.count; i++) {
                MarketEngineeringItemMo *tmpMo = self.arrMarketItem[i];
...
//            rowMo.strValue = [NSString stringWithFormat:@"%lu", (long)self.arrReporting.count];
//            rowMo.value = [NSString stringWithFormat:@"%lu", (long)self.arrReporting.count];
//        }
//        else if ([rowMo.key isEqualToString:@"actualSign"]) {
//            NSInteger count = 0;
//            for (int i = 0; i < self.arrReporting.count; i++) {
//                SignIntentionMo *tmpMo = self.arrReporting[i];
//                if (tmpMo.sign) count++;
//            }
//            rowMo.strValue = [NSString stringWithFormat:@"%ld", (long)count];
//            rowMo.value = [NSString stringWithFormat:@"%ld", (long)count];
...
- (void)dealWithParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    for (MarketEngineeringItemMo *tmpMo in self.arrMarketItem) {
        [marketArr addObject:@{@"member": @{@"id":@([tmpMo.member[@"id"] longValue]),
                                            @"abbreviation":STRING(tmpMo.member[@"orgName"])},
                               @"visit":@(tmpMo.visit),
                               @"handlingMatters":STRING(tmpMo.handlingMatters)}];
    }
    [params setObject:marketArr forKey:@"marketEngineeringItems"];
    ...
    [params setObject:reportArr forKey:@"reportingProjects"];
    ...
    [params setObject:unfulArr forKey:@"unfulfilledProjects"];
...
- (void)createParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    [[JYUserApi sharedInstance] createDynmicFormDynamicId:@"market-engineering" param:params success:^(id responseObject) {
        [Utils dismissHUD];
...
- (void)updateParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    [[JYUserApi sharedInstance] updateDynmicFormDynamicId:@"market-engineering" param:params success:^(id responseObject) {
        [Utils dismissHUD];
