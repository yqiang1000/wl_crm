...
@interface JinMuMenPageViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
...
@implementation JinMuMenPageViewCtrl

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"金木门销售人员计划";
    self.dateStr = @"";
    self.keyWord = @"";
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getPageDynmicFormDynamicId:@"jin-mu" param:param success:^(id responseObject) {
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
        NSMutableArray *tmpArr = [JinMuMenMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    JinMuMenMo *model = self.arrData[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    JinMuMenMo *model = self.arrData[indexPath.row];
    JinMuMenWorkPlanViewCtrl *vc = [[JinMuMenWorkPlanViewCtrl alloc] init];
    vc.model = model;
    vc.title = @"金木门销售人员计划";
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIAlertAction *action2 = [UIAlertAction actionWithTitle:GET_LANGUAGE_KEY(@"确定") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (model.id != 0) {
            ...
            [[JYUserApi sharedInstance] detailWorkPlanType:@"jin-mu" ids:@[@(model.id)] success:^(id responseObject) {
                [Utils dismissHUD];
