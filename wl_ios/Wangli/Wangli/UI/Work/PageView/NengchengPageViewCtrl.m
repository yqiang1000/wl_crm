...
@interface NengchengPageViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
...
@implementation NengchengPageViewCtrl
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getPageDynmicFormDynamicId:@"neng-cheng" param:param success:^(id responseObject) {
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
        NSMutableArray *tmpArr = [NengchengMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NengchengMo *model = self.arrData[indexPath.row];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIAlertAction *action2 = [UIAlertAction actionWithTitle:GET_LANGUAGE_KEY(@"确定") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (model.id != 0) {
            ...
            [[JYUserApi sharedInstance] detailWorkPlanType:@"neng-cheng" ids:@[@(model.id)] success:^(id responseObject) {
                [Utils dismissHUD];
