...
@interface RetailPageViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
...
@implementation RetailPageViewCtrl
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getRetailChannelPageParam:param success:^(id responseObject) {
        ...
        NSError *error = nil;
        NSMutableArray *tmpArr = [RetailChannelMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    RetailChannelMo *model = self.arrData[indexPath.row];
    NSString *name = model.operator[@"name"];
    cell.labName.text = name.length == 0 ? @"暂无业务员" : name;
    cell.labTime.text = STRING(model.workPlanDate);
    cell.labKTL.text = @"当月目标";
    cell.labKTR.text = @"当日计划";
    cell.labKBL.text = @"当月发货量";
    cell.labKBR.text = @"当日实发";
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIAlertAction *action2 = [UIAlertAction actionWithTitle:GET_LANGUAGE_KEY(@"确定") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (model.id != 0) {
            ...
            [[JYUserApi sharedInstance] detailWorkPlanType:@"retail-channel" ids:@[@(model.id)] success:^(id responseObject) {
                [Utils dismissHUD];
