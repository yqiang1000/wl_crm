...
@interface ChannelDevelopPageViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
...
@implementation ChannelDevelopPageViewCtrl
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getChannelDevelopPageParam:param success:^(id responseObject) {
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
        NSMutableArray *tmpArr = [ChannelDevelopmentMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ChannelDevelopmentMo *model = self.arrData[indexPath.row];
    NSString *name = model.operator[@"name"];
    cell.labName.text = name.length == 0 ? @"暂无业务员" : name;
    cell.labTime.text = STRING(model.workPlanDate);
    cell.labKTL.text = @"当月开发目标";
    cell.labKTR.text = @"当日拜访意向客户";
    cell.labKBL.text = @"当月累计拜访量";
    cell.labKBR.text = @"当日完成拜访";
    cell.labTL.text = [NSString stringWithFormat:@"%ld", (long)model.developmentProject];
    cell.labTR.text = [NSString stringWithFormat:@"%ld", (long)model.visitIntentional];
    cell.labBL.text = [NSString stringWithFormat:@"%ld", (long)model.accumulateVisit];
    cell.labBR.text = [NSString stringWithFormat:@"%ld", (long)model.finishVisit];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIAlertAction *action2 = [UIAlertAction actionWithTitle:GET_LANGUAGE_KEY(@"确定") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (model.id != 0) {
            ...
            [[JYUserApi sharedInstance] detailWorkPlanType:@"channel-development" ids:@[@(model.id)] success:^(id responseObject) {
                [Utils dismissHUD];
