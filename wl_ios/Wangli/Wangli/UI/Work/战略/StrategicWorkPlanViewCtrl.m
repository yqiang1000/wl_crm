...
#import "StrategicEngineeringItemViewCtrl.h"

@interface StrategicWorkPlanViewCtrl () <UITableViewDelegate, UITableViewDataSource, ListSelectViewCtrlDelegate, MyTextViewCellDelegate, DefaultInputCellDelegate, MemberSelectViewCtrlDelegate, JYUserMoSelectViewCtrlDelegate, MySwitchCellDelegate>
...
@implementation StrategicWorkPlanViewCtrl
...
- (void)getYesterDayDataWorkType {
    ...
    [[JYUserApi sharedInstance] getYesterdayWorkDataWorkType:WORKYTPE_STR_ENGIN param:@{@"loginOperator":@(TheUser.userMo.id), @"yesterday":yesterdayStr} success:^(id responseObject) {
        [Utils dismissHUD];
        NSError *error = nil;
        self.model = [[StrategicEngineeringMo alloc] initWithDictionary:responseObject error:&error];
        self.createDate = YES;
...
- (void)getDetailModel {
    [Utils showHUDWithStatus:nil];
    [[JYUserApi sharedInstance] getStrategicEngineeringDetail:self.model.id param:nil success:^(id responseObject) {
        [Utils dismissHUD];
        NSError *error = nil;
        self.model = [[StrategicEngineeringMo alloc] initWithDictionary:responseObject error:&error];
...
//    NSDate *nextDay = [NSDate dateWithTimeInterval:24*60*60 sinceDate:today];//后一天
    ...
    _arrMarketItem = nil;
    for (int i = 0; i < self.model.strategicEngineeringItems.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.strategicEngineeringItems[i];
        StrategicEngineeringItemMo *tmpMo = [[StrategicEngineeringItemMo alloc] initWithDictionary:dic error:&error];
...
- (void)getProvinceList {
    ...
    [[JYUserApi sharedInstance] getWorkPlanTargetByOperatorType:@"3"param:@{@"id":@(userMo.id),@"oaCode":STRING(userMo.oaCode)} success:^(id responseObject) {
...
- (void)getMonthTotalSum {
    ...
    [[JYUserApi sharedInstance] getWorkSumAcutalShipmentType:@"strategic-engineering" province:STRING(provinceRowMo.strValue) param:@{@"id":@(userMo.id)} success:^(id responseObject) {
...
//        return;
//    }
//    [[JYUserApi sharedInstance] getMarketEngineeringSumAccumulateVisitParam:@{@"id":@(userMo.id)} success:^(id responseObject) {
//        CommonRowMo *rowMo = self.arrData[2];
//        NSInteger sumActualShipment = [responseObject[@"sumActualShipment"] floatValue];
//        rowMo.strValue = [NSString stringWithFormat:@"%ld", (long)sumActualShipment];
//        rowMo.value = [NSString stringWithFormat:@"%ld", (long)sumActualShipment];
//        [self dealWithSum];
//    } failure:^(NSError *error) {
//        CommonRowMo *rowMo = self.arrData[2];
//        CGFloat sumActualShipment = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        if (indexPath.section == 1) {
            StrategicEngineeringItemMo *tmpMo = self.arrMarketItem[indexPath.row];
            cell.labTitle.text = tmpMo.constructionSite;
        }
        return cell;
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        StrategicEngineeringItemMo *itemMo = self.arrMarketItem[indexPath.row];
        StrategicEngineeringItemViewCtrl *vc = [[StrategicEngineeringItemViewCtrl alloc] init];
...
- (void)createMarket:(UIButton *)sender {
    ...
    vc.afterDate = self.afterDate;
    __weak typeof(self) weakself = self;
    vc.updateSuccess = ^(StrategicEngineeringItemMo *tmpMo) {
        __strong typeof(self) strongself = weakself;
    ...
    for (CommonRowMo *rowMo in self.arrData) {
        if ([rowMo.key isEqualToString:@"expectVisit"]) {
            ...
            for (int i = 0; i < self.arrMarketItem.count; i++) {
                StrategicEngineeringItemMo *tmpMo = self.arrMarketItem[i];
...
- (void)dealWithParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    for (StrategicEngineeringItemMo *tmpMo in self.arrMarketItem) {
        [marketArr addObject:@{@"constructionSite": STRING(tmpMo.constructionSite),
                               @"visit":@(tmpMo.visit),
                               @"achieveResults":STRING(tmpMo.achieveResults)}];
    }
    [params setObject:marketArr forKey:@"strategicEngineeringItems"];
...
- (void)createParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    [[JYUserApi sharedInstance] createDynmicFormDynamicId:@"strategic-engineering" param:params success:^(id responseObject) {
        [Utils dismissHUD];
...
- (void)updateParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    [[JYUserApi sharedInstance] updateDynmicFormDynamicId:@"strategic-engineering" param:params success:^(id responseObject) {
        [Utils dismissHUD];
