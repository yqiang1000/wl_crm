//
//  StrategicEngineeringItemViewCtrl.m
//  Wangli
//
...
@interface StrategicEngineeringItemViewCtrl ()

@end

@implementation StrategicEngineeringItemViewCtrl
...
- (void)configRowMos {
    ...
    rowMo0.leftContent = @"工地/地产";
    rowMo0.inputType = K_SHORT_TEXT;
    rowMo0.rightContent = @"请输入";
    rowMo0.editAble = !self.afterDate;
    rowMo0.key = @"constructionSite";
    rowMo0.m_obj = STRING(self.itemMo.constructionSite);
    rowMo0.strValue = STRING(self.itemMo.constructionSite);
...
- (void)updateParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    for (CommonRowMo *rowMo in self.arrData) {
        if ([rowMo.key isEqualToString:@"constructionSite"]) {
            self.itemMo.constructionSite = STRING(rowMo.strValue);
...
- (StrategicEngineeringItemMo *)itemMo {
    if (!_itemMo) _itemMo = [[StrategicEngineeringItemMo alloc] init];
    return _itemMo;
}
