...
#import "JYReportingProjectItemViewCtrl.h"
#import "JYUnfulFilledProjectViewCtrl.h"
#import "JYVisitViewCtrl.h"
#import "JYSignViewCtrl.h"

@interface JYWorkPlanViewCtrl () <UITableViewDelegate, UITableViewDataSource, ListSelectViewCtrlDelegate, MyTextViewCellDelegate, DefaultInputCellDelegate, MemberSelectViewCtrlDelegate, JYUserMoSelectViewCtrlDelegate, MySwitchCellDelegate>
...
@property (nonatomic, strong) NSMutableArray *arrWorkItem;
...
@property (nonatomic, copy) NSString *salesTargetStr;
@property (nonatomic, copy) NSString *developTargetStr;

@end

@implementation JYWorkPlanViewCtrl
...
- (void)viewDidLoad {
    ...
    NSLog(@"%@ ---- %ld", self.title, self.workType);
    [self setUI];
    if (self.model) {
...
- (void)getYesterDayDataWorkType {
    ...
    switch (self.workType) {
        case JYWorkTypeJinMuMen:
            workTypeStr = WORKTYPE_GOLDWOOD;
            break;
        case JYWorkTypeLvMuMen:
            workTypeStr = WORKTYPE_ALD;
            break;
        case JYWorkTypeZhiNengSuo:
            workTypeStr = WORKTYPE_ZHINENG;
            break;
        case JYWorkTypeTongMuMen:
            workTypeStr = WORKTYPE_CERART;
            break;
        case JYWorkTypeMuMen:
            workTypeStr = WORKTYPE_TIMBER;
            break;
        default:
            break;
    }
    [[JYUserApi sharedInstance] getYesterdayWorkDataCommonWorkType:workTypeStr param:@{@"loginOperator":@(TheUser.userMo.id), @"yesterday":yesterdayStr} success:^(id responseObject) {
        [Utils dismissHUD];
        NSError *error = nil;
        self.model = [[JYWorkMo alloc] initWithDictionary:responseObject error:&error];
        self.createDate = YES;
...
- (void)getDetailModel {
    [Utils showHUDWithStatus:nil];
    [[JYUserApi sharedInstance] getCommonWorkPlanById:self.model.id param:nil success:^(id responseObject) {
        [Utils dismissHUD];
        NSError *error = nil;
        self.model = [[JYWorkMo alloc] initWithDictionary:responseObject error:&error];
...
- (void)configRowMos {
    ...
    rowMo18.key = @"date";
    rowMo18.strValue = self.model?self.model.date:@"";//[today stringWithFormat:rowMo18.pattern];
    ...
    _arrSign = nil;
    for (int i = 0; i < self.model.workPlanItems.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.workPlanItems[i];
        JYWorkItemMo *tmpMo = [[JYWorkItemMo alloc] initWithDictionary:dic error:&error];
        [self.arrWorkItem addObject:tmpMo];
        [self loadAttachement:tmpMo];
    }
    for (int i = 0; i < self.model.workPlanReportingProjects.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.workPlanReportingProjects[i];
        JYReportingProjectMo *tmpMo = [[JYReportingProjectMo alloc] initWithDictionary:dic error:&error];
        [self.arrReporting addObject:tmpMo];
    }
    for (int i = 0; i < self.model.workPlanUnfulfilledProjects.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.workPlanUnfulfilledProjects[i];
        JYUnfulfilledProjectMo *tmpMo = [[JYUnfulfilledProjectMo alloc] initWithDictionary:dic error:&error];
        [self.arrUnFulFilled addObject:tmpMo];
    }
    for (int i = 0; i < self.model.workPlanVisitIntentions.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.workPlanVisitIntentions[i];
        JYVisitIntentionMo *tmpMo = [[JYVisitIntentionMo alloc] initWithDictionary:dic error:&error];
        [self.arrVisit addObject:tmpMo];
    }
    for (int i = 0; i < self.model.workPlanSignIntentions.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.workPlanSignIntentions[i];
        JYSignIntentionMo *tmpMo = [[JYSignIntentionMo alloc] initWithDictionary:dic error:&error];
...
- (void)loadAttachement:(JYWorkItemMo *)item {
    [[JYUserApi sharedInstance] getWorkPlanItemDetail:item.id param:nil success:^(id responseObject) {
        NSError *error = nil;
...
- (void)getProvinceList {
    ...
    [[JYUserApi sharedInstance] getWorkPlanTargetAreaByOperatorType:STRING(self.salesTargetStr) param:@{@"id":@(userMo.id),@"oaCode":STRING(userMo.oaCode)} success:^(id responseObject) {
        ...
        for (int i = 0; i < responseData.count; i++) {
            ...
            if (self.workType == JYWorkTypeMuMen) {
                dicMo.key = [NSString stringWithFormat:@"%@-%@-%@", dic[@"province"], dic[@"brand"][@"brandName"], dic[@"type"]];
            } else {
        ...
        if (self.workType == JYWorkTypeMuMen) [self getMumenProvinceList];
    } failure:^(NSError *error) {
        [_arrProvince removeAllObjects];
        _arrProvince = nil;
        [Utils showToastMessage:STRING(error.userInfo[@"message"])];
...
- (void)getMumenProvinceList {
    CommonRowMo *rowMo = self.arrData[0];
    JYUserMo *userMo = (JYUserMo *)rowMo.m_obj;
    if (!userMo) {
        return;
    }
    [[JYUserApi sharedInstance] getWorkPlanTargetAreaByOperatorType:@"1" param:@{@"id":@(userMo.id),@"oaCode":STRING(userMo.oaCode)} success:^(id responseObject) {
        NSArray *responseData = (NSArray *)responseObject;
        for (int i = 0; i < responseData.count; i++) {
            NSDictionary *dic = responseData[i];
            DicMo *dicMo = [[DicMo alloc] init];
...
- (void)getDevelopProvinceList {
    ...
    [[JYUserApi sharedInstance] getWorkPlanTargetAreaByOperatorType:STRING(self.developTargetStr) param:@{@"id":@(userMo.id),@"oaCode":STRING(userMo.oaCode)} success:^(id responseObject) {
        NSArray *responseData = (NSArray *)responseObject;
...
//    }
//
//    [[JYUserApi sharedInstance] getWorkPlanSumAcutalShipmentType:self.workTypeStr province:STRING(provinceRowMo.strValue) param:@{@"id":@(userMo.id)} success:^(id responseObject) {
...
- (void)getDevelopMonthTotalSum {
    ...
    [[JYUserApi sharedInstance] getWorkPlanSumAccumulateVisitProvince:nil param:param success:^(id responseObject) {
...
//    }
//
//    [[JYUserApi sharedInstance] getWorkPlanSumAccumulateVisitProvince:STRING(provinceRowMo.strValue) param:@{@"id":@(userMo.id)} success:^(id responseObject) {
//        CGFloat sumActualShipment = [STRING(responseObject[@"sumAccumulateVisit"]) floatValue];
//        sumRowMo.strValue = [Utils getPriceFrom:sumActualShipment];
//        sumRowMo.value = [NSString stringWithFormat:@"%f", sumActualShipment];
//        [self dealWithDevelopSum];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 1) return self.arrWorkItem.count;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        static NSString *identifier = @"JYWorkPlanItemCell";
        JYWorkPlanItemCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
        if (!cell) {
            cell = [[JYWorkPlanItemCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
        }
        if (indexPath.section == 1) {
            JYWorkItemMo *tmpMo = self.arrWorkItem[indexPath.row];
            ...
            JYReportingProjectMo *tmpMo = self.arrReporting[indexPath.row];
            cell.labTitle.text = tmpMo.engineering;
        } else if (indexPath.section == 3) {
            JYUnfulfilledProjectMo *tmpMo = self.arrUnFulFilled[indexPath.row];
            cell.labTitle.text = tmpMo.engineering;
        } else if (indexPath.section == 4) {
            JYVisitIntentionMo *tmpMo = self.arrVisit[indexPath.row];
            cell.labTitle.text = tmpMo.member;
        } else if (indexPath.section == 5) {
            JYSignIntentionMo *tmpMo = self.arrSign[indexPath.row];
            cell.labTitle.text = tmpMo.member;
        }
        return cell;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        JYWorkItemMo *itemMo = self.arrWorkItem[indexPath.row];
        JYWorkItemViewCtrl *vc = [[JYWorkItemViewCtrl alloc] init];
        ...
        JYReportingProjectMo *itemMo = self.arrReporting[indexPath.row];
        JYReportingProjectItemViewCtrl *vc = [[JYReportingProjectItemViewCtrl alloc] init];
        vc.createDate = self.createDate;
        vc.beforeDate = self.beforeDate;
        vc.handleDate = self.handleDate;
        vc.afterDate = self.afterDate;
        ...
        JYUnfulfilledProjectMo *itemMo = self.arrUnFulFilled[indexPath.row];
        JYUnfulFilledProjectViewCtrl *vc = [[JYUnfulFilledProjectViewCtrl alloc] init];
        vc.createDate = self.createDate;
        vc.beforeDate = self.beforeDate;
        vc.handleDate = self.handleDate;
        vc.afterDate = self.afterDate;
        ...
        JYVisitIntentionMo *itemMo = self.arrVisit[indexPath.row];
        JYVisitViewCtrl *vc = [[JYVisitViewCtrl alloc] init];
        vc.createDate = self.createDate;
        vc.beforeDate = self.beforeDate;
        vc.handleDate = self.handleDate;
        vc.afterDate = self.afterDate;
        ...
        JYSignIntentionMo *itemMo = self.arrSign[indexPath.row];
        JYSignViewCtrl *vc = [[JYSignViewCtrl alloc] init];
        vc.createDate = self.createDate;
        vc.beforeDate = self.beforeDate;
        vc.handleDate = self.handleDate;
        vc.afterDate = self.afterDate;
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIAlertAction *action2 = [UIAlertAction actionWithTitle:GET_LANGUAGE_KEY(@"确定") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if (indexPath.section == 1) {
            [self.arrWorkItem removeObjectAtIndex:indexPath.row];
...
- (void)getSapData:(NSMutableArray *)provinceData {
    ...
    [[JYUserApi sharedInstance] GET_SAP_SALES_BY_WORK_PLAN_Param:param success:^(id responseObject) {
...
- (void)getCustomersByAddress {
    [Utils showHUDWithStatus:@"处理中，请稍后..."];
    [_arrWorkItem removeAllObjects];
    _arrWorkItem = nil;
    ...
    [[JYUserApi sharedInstance] findCarefulAreaParam:param success:^(id responseObject) {
        ...
        for (NSDictionary *tmpDic in tmpArr) {
            JYWorkItemMo *tmpMo = [[JYWorkItemMo alloc] init];
...
- (void)getCustomersByAddress1 {
    ...
    [[JYUserApi sharedInstance] getMarketEngineeringRequireEngineerParam:unfullParam success:^(id responseObject) {
        ...
        for (NSDictionary *tmpDic in responseObject) {
            JYUnfulfilledProjectMo *tmpMo = [[JYUnfulfilledProjectMo alloc] init];
...
- (void)createNengchengItem:(UIButton *)sender {
    ...
    vc.afterDate = self.afterDate;
    __weak typeof(self) weakself = self;
    vc.updateSuccess = ^(JYWorkItemMo *tmpMo) {
        __strong typeof(self) strongself = weakself;
        [strongself.arrWorkItem addObject:tmpMo];
...
- (void)createReport:(UIButton *)sender {
    ...
    vc.afterDate = self.afterDate;
    __weak typeof(self) weakself = self;
    vc.updateSuccess = ^(JYReportingProjectMo *tmpMo) {
        __strong typeof(self) strongself = weakself;
...
- (void)createUnFul:(UIButton *)sender {
    ...
    vc.afterDate = self.afterDate;
    __weak typeof(self) weakself = self;
    vc.updateSuccess = ^(JYUnfulfilledProjectMo *tmpMo) {
        __strong typeof(self) strongself = weakself;
...
- (void)createVisit:(UIButton *)sender {
    ...
    vc.afterDate = self.afterDate;
    __weak typeof(self) weakself = self;
    vc.updateSuccess = ^(JYVisitIntentionMo *tmpMo) {
        __strong typeof(self) strongself = weakself;
...
- (void)createSign:(UIButton *)sender {
    ...
    vc.afterDate = self.afterDate;
    __weak typeof(self) weakself = self;
    vc.updateSuccess = ^(JYSignIntentionMo *tmpMo) {
        __strong typeof(self) strongself = weakself;
    ...
    for (CommonRowMo *rowMo in self.arrData) {
        if ([rowMo.key isEqualToString:@"visitIntentional"]) {
            ...
            for (int i = 0; i < self.arrVisit.count; i++) {
                JYVisitIntentionMo *tmpMo = self.arrVisit[i];
...
- (void)updateNengchengCount {
    for (CommonRowMo *rowMo in self.arrData) {
        if ([rowMo.key isEqualToString:@"expectVisit"]) {
            rowMo.strValue = [NSString stringWithFormat:@"%lu", (long)self.arrWorkItem.count];
            rowMo.value = [NSString stringWithFormat:@"%lu", (long)self.arrWorkItem.count];
        }
        else if ([rowMo.key isEqualToString:@"actualVisit"]) {
            NSInteger count = 0;
            for (int i = 0; i < self.arrWorkItem.count; i++) {
                JYWorkItemMo *tmpMo = self.arrWorkItem[i];
                if (tmpMo.visit) count++;
            }
            rowMo.strValue = [NSString stringWithFormat:@"%ld", (long)count];
            rowMo.value = [NSString stringWithFormat:@"%ld", (long)count];
...
- (void)clickRightButton:(UIButton *)sender {
    ...
    if (self.arrWorkItem.count == 0) {
        [Utils showToastMessage:@"走访客户数量不能为0"];
        return;
    }
    
    [param setObject:STRING(self.workTypeStr) forKey:@"workPlanType"];
...
- (void)dealWithParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    for (JYWorkItemMo *tmpMo in self.arrWorkItem) {
    ...
    [params setObject:memberArr forKey:@"workPlanItems"];// nengChengItems
    // 报备工程跟进
    NSMutableArray *reportArr = [NSMutableArray new];
    for (JYReportingProjectMo *tmpMo in self.arrReporting) {
    ...
    [params setObject:reportArr forKey:@"workPlanReportingProjects"];
    
    // 未履行工程
    NSMutableArray *unfulArr = [NSMutableArray new];
    for (JYUnfulfilledProjectMo *tmpMo in self.arrUnFulFilled) {
    ...
    [params setObject:unfulArr forKey:@"workPlanUnfulfilledProjects"];
    
    // 拜访意向客户
    NSMutableArray *visitArr = [NSMutableArray new];
    for (JYVisitIntentionMo *tmpMo in self.arrVisit) {
    ...
    [params setObject:visitArr forKey:@"workPlanVisitIntentions"];
    
    // 签署客户
    NSMutableArray *signArr = [NSMutableArray new];
    for (JYSignIntentionMo *tmpMo in self.arrSign) {
        [signArr addObject:@{@"member": STRING(tmpMo.member),
                             @"sign":@(tmpMo.sign)}];
    }
    [params setObject:signArr forKey:@"workPlanSignIntentions"];
    [params setObject:STRING(self.workTypeStr) forKey:@"workPlanType"];
...
- (void)createParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    [[JYUserApi sharedInstance] createDynmicFormDynamicId:@"workPlan" param:params success:^(id responseObject) {
        [Utils dismissHUD];
...
- (void)updateParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    [[JYUserApi sharedInstance] updateDynmicFormDynamicId:@"workPlan" param:params success:^(id responseObject) {
        [Utils dismissHUD];
...
- (NSMutableArray *)arrWorkItem {
    if (!_arrWorkItem) _arrWorkItem = [NSMutableArray new];
    return _arrWorkItem;
}
...
- (void)setWorkType:(JYWorkType)workType {
    ...
    switch (_workType) {
        case JYWorkTypeJinMuMen: self.workTypeStr = @"GOLDWOOD";
            self.developTargetStr = @"2";
            self.salesTargetStr = @"3";
            break;
        case JYWorkTypeLvMuMen: self.workTypeStr = @"ALD";
            self.developTargetStr = @"2";
            self.salesTargetStr = @"0";
            break;
