...
@interface JinMuMenWorkPlanViewCtrl () <UITableViewDelegate, UITableViewDataSource, ListSelectViewCtrlDelegate, MyTextViewCellDelegate, DefaultInputCellDelegate, MemberSelectViewCtrlDelegate, JYUserMoSelectViewCtrlDelegate, MySwitchCellDelegate>
...
@implementation JinMuMenWorkPlanViewCtrl
...
- (void)getDetailModel {
    ...
    [[JYUserApi sharedInstance] getJinMuMenDetail:self.model.id param:nil success:^(id responseObject) {
        [Utils dismissHUD];
        NSError *error = nil;
        self.model = [[JinMuMenMo alloc] initWithDictionary:responseObject error:&error];
...
- (void)configRowMos {
    ...
    // 如果新建,或次日，不可编辑，上班之后可以编辑
    rowMo9.editAble = (self.createDate||self.afterDate)?NO:YES;
    // 如果新建为空，之后可为空可不为空
    rowMo9.nullAble = self.createDate;
...
//    NSDate *nextDay = [NSDate dateWithTimeInterval:24*60*60 sinceDate:today];//后一天
    ...
    _arrUnFulFilled = nil;
    for (int i = 0; i < self.model.jinMuMenItems.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.jinMuMenItems[i];
    ...
    for (int i = 0; i < self.model.jmReportingProjects.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.jmReportingProjects[i];
        ReportingProjectMo *tmpMo = [[ReportingProjectMo alloc] initWithDictionary:dic error:&error];
        [self.arrReporting addObject:tmpMo];
    }
    for (int i = 0; i < self.model.jmUnfulfilledProjects.count; i++) {
        error = nil;
        NSDictionary *dic = self.model.jmUnfulfilledProjects[i];
...
- (void)loadAttachement:(NengChengItemMo *)item {
    [[JYUserApi sharedInstance] getJinMuMenItemDetail:item.id param:nil success:^(id responseObject) {
        NSError *error = nil;
...
- (void)getProvinceList {
    ...
    [[JYUserApi sharedInstance] getWorkPlanTargetByOperatorType:@"0" param:@{@"id":@(userMo.id),@"oaCode":STRING(userMo.oaCode)} success:^(id responseObject) {
...
- (void)getMonthTotalSum {
    ...
    [[JYUserApi sharedInstance] getWorkSumAcutalShipmentType:@"jin-mu" province:STRING(provinceRowMo.strValue) param:@{@"id":@(userMo.id)} success:^(id responseObject) {
...
- (void)dealWithParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    [params setObject:memberArr forKey:@"jinMuMenItems"];
    ...
    [params setObject:reportArr forKey:@"jmReportingProjects"];
    ...
    [params setObject:unfulArr forKey:@"jmUnfulfilledProjects"];
...
- (void)createParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    [[JYUserApi sharedInstance] createDynmicFormDynamicId:@"jin-mu" param:params success:^(id responseObject) {
        [Utils dismissHUD];
...
- (void)updateParams:(NSMutableDictionary *)params attachementList:(NSArray *)attachementList {
    ...
    [[JYUserApi sharedInstance] updateDynmicFormDynamicId:@"jin-mu" param:params success:^(id responseObject) {
        [Utils dismissHUD];
