//
//  CreateProductionLicenseViewCtrl.m
//  Wangli
//
...
@interface CreateProductionLicenseViewCtrl ()
...
@implementation CreateProductionLicenseViewCtrl
...
- (void)config {
    ...
    NSArray *keys = @[@"production_license",
                      @"procurement_bidding",
                      @"import_and_export_information",
                      @"tax_rating",
                      @"spot_check"];
    [[JYUserApi sharedInstance] getConfigDicByName:@"message_type" key:keys[_type] success:^(id responseObject) {
        self.dicMo = [[DicMo alloc] initWithDictionary:responseObject error:nil];
...
//        [[JYUserApi sharedInstance] getConfigDicByName:@"message_type" success:^(id responseObject) {
...
//                [self.selectShowArr addObject:STRING(tmpMo.value)];
//            }
...
- (void)listSelectViewCtrl:(ListSelectViewCtrl *)listSelectViewCtrl selectIndexPaths:(NSArray *)selectIndexPaths indexPath:(NSIndexPath *)indexPath {
    [listSelectViewCtrl.navigationController popViewControllerAnimated:YES];
    NSIndexPath *selectIndex = [selectIndexPaths firstObject];
    
    [self.arrValues replaceObjectAtIndex:indexPath.row withObject:STRING([Utils saveToValues:self.selectShowArr[selectIndex.row]])];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationFade];
    self.dicMo = self.selectArr[selectIndex.row];
}
...
//    kSpotCheckType,             // 抽查检查
    if (_mo) {
        if (_type == kLicenseType) {
            [[JYUserApi sharedInstance] updateLicenseId:_mo.id param:params attachments:[Utils filterUrls:attachments arrFile:_mo.attachments] success:^(id responseObject) {
                [self updateResponseObject:responseObject error:nil];
            } failure:^(NSError *error) {
                [self updateResponseObject:nil error:error];
            }];
        }
        else if (_type == kPruchaseType) {
            [[JYUserApi sharedInstance] updatePruchaseId:_mo.id param:params attachments:[Utils filterUrls:attachments arrFile:_mo.attachments] success:^(id responseObject) {
                [self updateResponseObject:responseObject error:nil];
            } failure:^(NSError *error) {
                [self updateResponseObject:nil error:error];
            }];
        }
        else if (_type == kPortInfoType) {
            [[JYUserApi sharedInstance] updatePortInfoId:_mo.id param:params attachments:[Utils filterUrls:attachments arrFile:_mo.attachments] success:^(id responseObject) {
                [self updateResponseObject:responseObject error:nil];
            } failure:^(NSError *error) {
                [self updateResponseObject:nil error:error];
            }];
        }
        else if (_type == kTaxRatingType) {
            [[JYUserApi sharedInstance] updateTaxRatingId:_mo.id param:params attachments:[Utils filterUrls:attachments arrFile:_mo.attachments] success:^(id responseObject) {
                [self updateResponseObject:responseObject error:nil];
            } failure:^(NSError *error) {
                [self updateResponseObject:nil error:error];
            }];
        }
        else if (_type == kSpotCheckType) {
            [[JYUserApi sharedInstance] updateSpotCheckId:_mo.id param:params attachments:[Utils filterUrls:attachments arrFile:_mo.attachments] success:^(id responseObject) {
                [self updateResponseObject:responseObject error:nil];
            } failure:^(NSError *error) {
                [self updateResponseObject:nil error:error];
            }];
        ...
        if (_type == kLicenseType) {
            [[JYUserApi sharedInstance] createLicenseParam:params attachments:attachments success:^(id responseObject) {
                [self createResponseObject:responseObject error:nil];
            } failure:^(NSError *error) {
                [self createResponseObject:nil error:error];
            }];
        }
        else if (_type == kPruchaseType) {
            [[JYUserApi sharedInstance] createPruchaseParam:params attachments:attachments success:^(id responseObject) {
                [self createResponseObject:responseObject error:nil];
            } failure:^(NSError *error) {
                [self createResponseObject:nil error:error];
            }];
            ...
            [[JYUserApi sharedInstance] createPortInfoParam:params attachments:attachments success:^(id responseObject) {
                [self createResponseObject:responseObject error:nil];
            } failure:^(NSError *error) {
                [self createResponseObject:nil error:error];
            }];
            ...
            [[JYUserApi sharedInstance] createTaxRatingParam:params attachments:attachments success:^(id responseObject) {
                [self createResponseObject:responseObject error:nil];
            } failure:^(NSError *error) {
                [self createResponseObject:nil error:error];
            }];
            ...
            [[JYUserApi sharedInstance] createSpotCheckParam:params attachments:attachments success:^(id responseObject) {
                [self createResponseObject:responseObject error:nil];
            } failure:^(NSError *error) {
                [self createResponseObject:nil error:error];
            }];
...
- (void)createResponseObject:(id)responseObject error:(NSError *)error {
    if (error) {
        [Utils dismissHUD];
        [Utils showToastMessage:STRING(error.userInfo[@"message"])];
    } else {
...
- (void)updateResponseObject:(id)responseObject error:(NSError *)error {
    if (error) {
        [Utils dismissHUD];
        [Utils showToastMessage:STRING(error.userInfo[@"message"])];
    } else {
