...
#import "ProduceStatusMo.h"

@interface ProducePageStatusViewCtrl ()

@end

@implementation ProducePageStatusViewCtrl
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ProduceStatusMo *model = self.arrData[indexPath.row];
    [cell.btnFile setTitle:STRING(model.putBatch) forState:UIControlStateNormal];
    cell.labType.text = [NSString stringWithFormat:@"产品类型：%@", STRING(model.productType)];
    cell.labTime.text = [NSString stringWithFormat:@"投产日期：%@", STRING(model.putDate)];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    vc.isUpdate = YES;
    vc.fromTab = NO;
    vc.dynamicId = K_PRODUCT_PUTINTO;
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getPageDynmicFormDynamicId:K_PRODUCT_PUTINTO param:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
//        self.totalElements = [responseObject[@"totalElements"] integerValue];
        NSMutableArray *tmpArr = [ProduceStatusMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
