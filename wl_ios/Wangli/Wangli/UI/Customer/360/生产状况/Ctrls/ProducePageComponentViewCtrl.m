//
//  ProducePageComponentViewCtrl.m
//  Wangli
//
//  Created by yeqiang on 2019/1/4.
...
#import "ProduceComponentMo.h"

@interface ProducePageComponentViewCtrl ()

@end

@implementation ProducePageComponentViewCtrl
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ProduceComponentMo *model = self.arrData[indexPath.row];
    [cell.btnFile setTitle:STRING(model.projectValue) forState:UIControlStateNormal];
    cell.labType.text = [NSString stringWithFormat:@"测试条件：%@", STRING(model.condition)];
    cell.labTime.text = [NSString stringWithFormat:@"测试标准：%@", STRING(model.standard)];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    vc.isUpdate = YES;
    vc.fromTab = NO;
    vc.dynamicId = K_PRODUCT_COMPONENT;
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getPageDynmicFormDynamicId:K_PRODUCT_COMPONENT param:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
//        self.totalElements = [responseObject[@"totalElements"] integerValue];
        NSMutableArray *tmpArr = [ProduceComponentMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
