...
@interface ProducePageStandardViewCtrl ()

@end

@implementation ProducePageStandardViewCtrl
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ProduceStandardMo *model = self.arrData[indexPath.row];
    [cell.btnFile setTitle:STRING(model.summary) forState:UIControlStateNormal];
    cell.labType.text = [NSString stringWithFormat:@"创建人：%@", STRING(model.operator[@"name"])];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    vc.isUpdate = YES;
    vc.fromTab = NO;
    vc.dynamicId = K_PRODUCT_STANDARD;
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getPageDynmicFormDynamicId:K_PRODUCT_STANDARD param:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
//        self.totalElements = [responseObject[@"totalElements"] integerValue];
        NSMutableArray *tmpArr = [ProduceStandardMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
