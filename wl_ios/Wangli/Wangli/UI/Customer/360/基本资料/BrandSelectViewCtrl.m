...
@interface BrandSelectViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
{
...
@implementation BrandSelectViewCtrl
...
- (void)getMemberList:(NSInteger)page {
    ...
    if (self.searchKey.length > 0) [rules addObject:@{@"field":@"brandDesc-brandName",
                                                      @"option":@"LIKE_ANYWHERE",
                                                      @"values":@[STRING(self.searchKey)]}];
    if (rules.count > 0) [param setObject:rules forKey:@"rules"];
    [[JYUserApi sharedInstance] getBrandPageParam:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self tableViewEndRefresh];
        NSError *error = nil;
        NSMutableArray *tmpArr = [BrandMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BrandMo *tmpMo = self.arrData[indexPath.row];
    cell.labText.text = STRING(tmpMo.brandName);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    BrandMo *tmpMo = self.arrData[indexPath.row];
    if ([self.arrSelect containsObject:tmpMo]) {
        [self.arrSelect removeObject:tmpMo];
    } else {
        [self.arrSelect addObject:tmpMo];
    }
...
- (void)clickRightButton:(UIButton *)sender {
    if (self.arrSelect.count == 0) {
        [Utils showToastMessage:@"至少选择一个品牌"];
        return;
    }
    
    if (_vcDelegate && [_vcDelegate respondsToSelector:@selector(brandSelectViewCtrl:didSelected:)]) {
        [_vcDelegate brandSelectViewCtrl:self didSelected:self.arrSelect];
    }
    [self.navigationController popViewControllerAnimated:YES];
}
