//
//  DevelopPageLiboratoryViewCtrl.m
//  Wangli
//
...
@interface DevelopPageLiboratoryViewCtrl ()

@end

@implementation DevelopPageLiboratoryViewCtrl
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DevelopLaboratoryMo *model = self.arrData[indexPath.row];
    [cell loadData:model];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    vc.isUpdate = YES;
    vc.fromTab = NO;
    vc.dynamicId = K_DEVELOP_LABORATORY;
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getPageDynmicFormDynamicId:K_DEVELOP_LABORATORY param:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
//        self.totalElements = [responseObject[@"totalElements"] integerValue];
        NSMutableArray *tmpArr = [DevelopLaboratoryMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
