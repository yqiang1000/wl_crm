...
@interface DevelopPagePatentViewCtrl ()

@end

@implementation DevelopPagePatentViewCtrl
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *identifier = @"DevelopPatentCell";
    DevelopPatentCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[DevelopPatentCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    DevelopPatentMo *model = self.arrData[indexPath.row];
    DevelopCommonCreateViewCtrl *vc = [[DevelopCommonCreateViewCtrl alloc] init];
    vc.isUpdate = YES;
    vc.fromTab = NO;
    vc.dynamicId = K_DEVELOP_PATENT;
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getPageDynmicFormDynamicId:K_DEVELOP_PATENT param:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
////        self.totalElements = [responseObject[@"totalElements"] integerValue];
        NSMutableArray *tmpArr = [DevelopPatentMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
