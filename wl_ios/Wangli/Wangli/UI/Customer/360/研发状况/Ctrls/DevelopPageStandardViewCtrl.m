...
@interface DevelopPageStandardViewCtrl ()

@end

@implementation DevelopPageStandardViewCtrl
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DevelopStandardMo *model = self.arrData[indexPath.row];
    [cell.btnFile setTitle:STRING(model.summary) forState:UIControlStateNormal];
    cell.labType.text = [NSString stringWithFormat:@"文件类型：%@", STRING(model.typeValue)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    vc.isUpdate = YES;
    vc.fromTab = NO;
    vc.dynamicId = K_DEVELOP_STANDARD;
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getPageDynmicFormDynamicId:K_DEVELOP_STANDARD param:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
////        self.totalElements = [responseObject[@"totalElements"] integerValue];
        NSMutableArray *tmpArr = [DevelopStandardMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
