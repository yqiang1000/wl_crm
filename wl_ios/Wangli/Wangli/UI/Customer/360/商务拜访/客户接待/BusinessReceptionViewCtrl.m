...
@interface BusinessReceptionViewCtrl () <UITableViewDataSource, UITableViewDelegate, FSCalendarDataSource, FSCalendarDelegate, UIGestureRecognizerDelegate>
{
    void * _KVOContext;
...
@implementation BusinessReceptionViewCtrl
...
- (void)getNewPageCalendarData {
    ...
    if (self.fromMy) {
        // 获取总的
        [[JYUserApi sharedInstance] getCurrentUserReceptionCalendarDateStr:[self.dateFormatter stringFromDate:self.calendar.currentPage] param:nil success:^(id responseObject) {
        ...
        // 获取360里面
        [[JYUserApi sharedInstance] getBusinessVisitReceptionCalendarMemberId:TheCustomer.customerMo.id dateStr:[self.dateFormatter stringFromDate:self.calendar.currentPage] param:nil success:^(id responseObject) {
            [_pageCalendarData removeAllObjects];
            _pageCalendarData = nil;
            [self dealNewPageCalendarData:responseObject[@"current"]];
            [self dealNewPageCalendarData:responseObject[@"pre"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell loadReceptionData:self.arrData[indexPath.row]];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    BusinessReceptionMo *model = self.arrData[indexPath.row];
    WebDetailViewCtrl *vc = [[WebDetailViewCtrl alloc] init];
    vc.titleStr = @"接待详情";
    vc.urlStr = [NSString stringWithFormat:@"%@id=%lld&token=%@", VISIT_RECEPTION_DETAIL, model.id, [Utils token]];
...
- (void)getDataByPage:(NSInteger)page {
    ...
    if (self.fromMy) {
        ...
        [rules addObject:@{@"field":@"createOperator.id",
                           @"option":@"EQ",
                           @"values":@[@(TheUser.userMo.id)]}];
        
        [param setObject:rules forKey:@"rules"];
        
        [[JYUserApi sharedInstance] getCurrentReceptionPageParam:param success:^(id responseObject) {
            [Utils dismissHUD];
            [self.tableView.mj_header endRefreshing];
            [self.tableView.mj_footer endRefreshing];
            NSError *error = nil;
            NSMutableArray *tmpArr = [BusinessReceptionMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
            if (page == 0) {
        ...
        [[JYUserApi sharedInstance] getBusinessVisitReceptionListByCalendarMemberId:TheCustomer.customerMo.id dateStr:[self.dateFormatter stringFromDate:self.currentDate] param:nil success:^(id responseObject) {
            [Utils dismissHUD];
            [self.tableView.mj_header endRefreshing];
            [self.tableView.mj_footer endRefreshing];
            NSError *error = nil;
            NSMutableArray *tmpArr = [BusinessReceptionMo arrayOfModelsFromDictionaries:responseObject error:&error];
            if (page == 0) {
                [_arrData removeAllObjects];
                _arrData = nil;
                self.arrData = tmpArr;
