...
- (instancetype)init
{
    self = [super init];
    if (self) {
        _viewSize = CGSizeMake(SCREEN_WIDTH, SCREEN_HEIGHT);//[UIApplication sharedApplication].delegate.window.bounds.size;
...
- (BOOL)startRecordingByView:(UIView *) recordView
{
    if (recordView != nil) {
...
- (void)writeVideoFrame
{
    ...
    dispatch_async(_render_queue, ^{
        ...
        if ([self.delegate respondsToSelector:@selector(writeBackgroundFrameInContext:)])
            [self.delegate writeBackgroundFrameInContext:&bitmapContext];
        if ([self.delegate respondsToSelector:@selector(recorderDuration:)])
            [self.delegate recorderDuration:elapsed];
...
//    }
//    if(SYSTEM_VERSION_LESS_THAN(@"8") && ([UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeLeft || [UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeRight)) {
