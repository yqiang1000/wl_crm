...
@interface PersonalSelectViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
{
...
@implementation PersonalSelectViewCtrl
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_vcDelegate && [_vcDelegate respondsToSelector:@selector(personalSelectViewCtrl:didSelect:indexPath:)]) {
            ContactMo *tmpMo = self.arrData[indexPath.row];
            [_vcDelegate personalSelectViewCtrl:self didSelect:tmpMo indexPath:self.indexPath];
        }
    }];
}
...
- (void)clickLeftButton:(UIButton *)sender {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_vcDelegate && [_vcDelegate respondsToSelector:@selector(personalSelectViewCtrlDismiss:)]) {
            [_vcDelegate personalSelectViewCtrlDismiss:self];
        }
    }];
}
...
- (SearchTopView *)searchView {
    if (!_searchView) {
        ...
        _searchView.searchTxtField.placeholder = @"请输入联系人姓名关键字";
    }
    return _searchView;
}
