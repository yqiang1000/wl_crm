...
@interface SalesPageSystemViewCtrl ()

@end

@implementation SalesPageSystemViewCtrl
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SalesSystemMo *model = self.arrData[indexPath.row];
    [cell.btnFile setTitle:STRING(model.fileName) forState:UIControlStateNormal];
    cell.labType.text = [NSString stringWithFormat:@"文件类型：%@", STRING(model.fileTypeValue)];
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getPageDynmicFormDynamicId:K_SALES_SYSTEM param:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
//        self.totalElements = [responseObject[@"totalElements"] integerValue];
        NSMutableArray *tmpArr = [SalesSystemMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
