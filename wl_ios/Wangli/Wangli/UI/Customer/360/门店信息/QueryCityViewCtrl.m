...
@interface QueryCityViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
{
...
@implementation QueryCityViewCtrl

- (void)viewDidLoad {
    [super viewDidLoad];
    [self.leftBtn setTitle:@"取消" forState:UIControlStateNormal];
    [self.leftBtn setImage:nil forState:UIControlStateNormal];
    self.title = self.typeId==0?@"请选择省份":(self.typeId==1?@"请选择城市":@"请选择地区");
    [self setUI];
...
- (void)getMemberList:(NSInteger)page {
    ...
    if (self.typeId == 1) {
//        [rules addObject:@{@"field": @"cityName",
//                           @"option": @"LIKE_ANYWHERE",
//                           @"values":@[STRING(_searchKey)]}];
        [rules addObject:@{@"field": @"provinceId",
                           @"option": @"EQ",
                           @"values":@[STRING(_searchId)]}];
    } else if (self.typeId == 2) {
//        [rules addObject:@{@"field": @"areaName",
//                           @"option": @"LIKE_ANYWHERE",
//                           @"values":@[STRING(_searchKey)]}];
        [rules addObject:@{@"field": @"cityId",
                           @"option": @"EQ",
                           @"values":@[STRING(_searchId)]}];
    }
    [param setObject:rules forKey:@"rules"];
    [[JYUserApi sharedInstance] queryCityPageType:self.typeId param:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
        NSMutableArray *tmpArr = [QueryAddressMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
            self.arrData = tmpArr;
        } else {
            if (tmpArr.count > 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    QueryAddressMo *tmpMo = self.arrData[indexPath.row];
    if (self.typeId == 1) {
        cell.labText.text = tmpMo.cityName;
    } else if (self.typeId == 2) {
        cell.labText.text = tmpMo.areaName;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    QueryAddressMo *tmpMo = self.arrData[indexPath.row];
    NSString *addId = self.typeId==1?tmpMo.cityId:tmpMo.areaId;
    if (self.updateSuccess) self.updateSuccess(addId);
