...
//
//  ContractPageContractViewCtrl.m
//  Wangli
//
...
#import "ContractContractCell.h"
#import "WSDatePickerView.h"

@interface ContractPageContractViewCtrl () <SwitchViewDelegate, FilterListViewDelegate>
...
@implementation ContractPageContractViewCtrl
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *identifier = @"ContractContractCell";
    ContractContractCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[ContractContractCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
    [cell loadDataWith:@""];
    return cell;
}
...
- (void)tableViewFooterRefreshAction {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        for (int i = 0; i < 5; i++) {
            CustomerMo *mo = [[CustomerMo alloc] init];
            mo.headUrl = @"1";
            mo.abbreviation = [NSString stringWithFormat:@"通威---%d", i+count];
            mo.transactionType = @"私企 | 大型 | 四川成都";
            mo.salesmanName = @"业务范围：硅片、电池片";
            mo.riskLevel = @"5";
            mo.lastTransactionDayCount = 20;
