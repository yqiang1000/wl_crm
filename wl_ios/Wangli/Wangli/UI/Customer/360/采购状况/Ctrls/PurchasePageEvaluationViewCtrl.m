//
//  PurchasePageEvaluationViewCtrl.m
//  Wangli
//
...
@interface PurchasePageEvaluationViewCtrl ()

@end

@implementation PurchasePageEvaluationViewCtrl
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PurchaseEvaluationMo *model = self.arrData[indexPath.row];
    [cell.btnFile setTitle:STRING(model.name) forState:UIControlStateNormal];
    cell.labType.text = [NSString stringWithFormat:@"文件类型：%@", model.typeValue];
    cell.labTime.text = [NSString stringWithFormat:@"更新日期：%@", model.lastModifiedDate];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    vc.isUpdate = YES;
    vc.fromTab = NO;
    vc.dynamicId = K_PURCHASE_EVALUATION;
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getPageDynmicFormDynamicId:K_PURCHASE_EVALUATION param:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
////        self.totalElements = [responseObject[@"totalElements"] integerValue];
        NSMutableArray *tmpArr = [PurchaseEvaluationMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
