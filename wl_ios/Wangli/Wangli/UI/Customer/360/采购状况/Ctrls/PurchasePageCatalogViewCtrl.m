...
@interface PurchasePageCatalogViewCtrl ()

@end

@implementation PurchasePageCatalogViewCtrl
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString *identifier = @"PurchaseCatalogCell";
    PurchaseCatalogCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[PurchaseCatalogCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    PurchaseCatalogMo *model = self.arrData[indexPath.row];
    PurchaseCommonCreateViewCtrl *vc = [[PurchaseCommonCreateViewCtrl alloc] init];
    vc.isUpdate = YES;
    vc.fromTab = NO;
    vc.dynamicId = K_PURCHASE_PRODUCT;
...
- (void)getList:(NSInteger)page {
    ...
    [[JYUserApi sharedInstance] getPageDynmicFormDynamicId:K_PURCHASE_PRODUCT param:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        NSError *error = nil;
//        self.totalElements = [responseObject[@"totalElements"] integerValue];
        NSMutableArray *tmpArr = [PurchaseCatalogMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
