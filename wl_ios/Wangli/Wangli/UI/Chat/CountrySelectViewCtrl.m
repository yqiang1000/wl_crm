...
@interface CountrySelectViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
{
...
@implementation CountrySelectViewCtrl
...
- (void)getMemberList:(NSInteger)page {
    ...
    if (self.searchKey.length > 0) [rules addObject:@{@"field":@"land1-landx",
                                                      @"option":@"LIKE_ANYWHERE",
                                                      @"values":@[STRING(self.searchKey)]}];
    if (rules.count > 0) [param setObject:rules forKey:@"rules"];
    [[JYUserApi sharedInstance] getCountryListParam:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self tableViewEndRefresh];
        NSError *error = nil;
        NSMutableArray *tmpArr = [CountryMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CountryMo *tmpMo = self.arrData[indexPath.row];
    cell.labText.text = tmpMo.landx;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_vcDelegate && [_vcDelegate respondsToSelector:@selector(countrySelectViewCtrl:didSelect:indexPath:)]) {
            CountryMo *tmpMo = self.arrData[indexPath.row];
            [_vcDelegate countrySelectViewCtrl:self didSelect:tmpMo indexPath:self.indexPath];
        }
    }];
}
...
- (void)clickLeftButton:(UIButton *)sender {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_vcDelegate && [_vcDelegate respondsToSelector:@selector(countrySelectViewCtrlDismiss:)]) {
            [_vcDelegate countrySelectViewCtrlDismiss:self];
        }
    }];
}
...
- (SearchTopView *)searchView {
    if (!_searchView) {
        ...
        _searchView.searchTxtField.placeholder = @"请输入国家名称或者国家码关键字";
    }
    return _searchView;
}
