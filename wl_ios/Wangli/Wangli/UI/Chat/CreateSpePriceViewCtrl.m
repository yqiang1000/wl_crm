...
@interface CreateSpePriceViewCtrl () <UITableViewDataSource, UITableViewDelegate, ListSelectViewCtrlDelegate, MyButtonCellDelegate, MemberSelectViewCtrlDelegate, MyTextViewCellDelegate>
...
@implementation CreateSpePriceViewCtrl
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == self.arrData.count) {
            ...
            if (!cell) {
                ...
                [cell.btnSave setTitle:@"提交特价申请" forState:UIControlStateNormal];
                cell.cellDelegate = self;
            }
            return cell;
        } else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        // 申请原因
            ...
            [[JYUserApi sharedInstance] getConfigDicByName:@"low_price_reason" success:^(id responseObject) {
...
//    }
    ...
    [param setObject:STRING(self.arrValues[1]) forKey:@"beginDate"];
    [param setObject:STRING(self.arrValues[2]) forKey:@"endDate"];
    [param setObject:STRING(self.arrValues[3]) forKey:@"lowReason"];
    [param setObject:[self.arrValues[4] isEqualToString:@"demo"] ? @"" : STRING(self.arrValues[4]) forKey:@"remark"];
    
    NSMutableArray *specialRecordItems = [NSMutableArray new];
    for (ProductMo *tmpMo in self.arrData) {
        NSDictionary *dic = @{@"batchNumber":STRING(tmpMo.batch),
                              @"grade":STRING(tmpMo.levelValue),
                              @"price":STRING(tmpMo.priceApply)};
        [specialRecordItems addObject:dic];
    }
    [param setObject:specialRecordItems forKey:@"specialRecordItems"];
    
    [Utils showHUDWithStatus:nil];
    [[JYUserApi sharedInstance] createSpecialRecordParam:param success:^(id responseObject) {
        [Utils dismissHUD];
    ...
    for (int i = 0; i < self.arrData.count; i++) {
        ...
        if ([tmpMo.batch isEqualToString:proMo.batch]) {
            if ([tmpMo.level isEqualToString:proMo.level]) {
                [Utils showToastMessage:@"同一个批号等级的产品不允许申请多个特价"];
                return;
            }
        }
    }
