...
@interface OrderSelectViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
{
...
@implementation OrderSelectViewCtrl
...
- (void)getMemberList:(NSInteger)page {
    ...
    NSMutableArray *arrSpecialConditions = [[NSMutableArray alloc] initWithObjects:STRING(_searchKey), nil];
    [[JYUserApi sharedInstance] getOrderSendPage:page toUserId:_toUserId size:10 rules:nil specialConditions:arrSpecialConditions success:^(id responseObject) {
        [Utils dismissHUD];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_VcDelegate && [_VcDelegate respondsToSelector:@selector(orderSelectViewCtrl:selectedModel:indexPath:)]) {
            OrderMo *tmpMo = self.arrData[indexPath.row];
            [_VcDelegate orderSelectViewCtrl:self selectedModel:tmpMo indexPath:self.indexPath];
        }
    }];
}
...
- (void)clickLeftButton:(UIButton *)sender {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_VcDelegate && [_VcDelegate respondsToSelector:@selector(orderSelectViewCtrlDismiss:)]) {
            [_VcDelegate orderSelectViewCtrlDismiss:self];
        }
    }];
}
...
- (SearchTopView *)searchView {
    if (!_searchView) {
        ...
        _searchView.searchTxtField.placeholder = @"搜索订单客户名/产品关键字等";
    }
    return _searchView;
}
