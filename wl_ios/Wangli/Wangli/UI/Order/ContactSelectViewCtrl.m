...
@interface ContactSelectViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
{
...
@implementation ContactSelectViewCtrl
...
- (void)getMemberList:(NSInteger)page {
    ...
    [rules addObject: @{@"field": @"name-member.orgName-member.abbreviation",
                        @"option": @"LIKE_ANYWHERE",
                        @"values":@[STRING(_searchKey)]}];
    
    [[JYUserApi sharedInstance] getContactListParam:param rules:rules toUserId:_toUserId success:^(id responseObject) {
        [Utils dismissHUD];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ContactMo *tmpMo = self.arrData[indexPath.row];
    NSString *orgName = tmpMo.member[@"abbreviation"];
    cell.labText.text = orgName.length == 0 ? [NSString stringWithFormat:@"%@", tmpMo.name] : [NSString stringWithFormat:@"%@-%@", orgName, tmpMo.name];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_VcDelegate && [_VcDelegate respondsToSelector:@selector(contactSelectViewCtrl:selectedModel:indexPath:)]) {
            ContactMo *tmpMo = self.arrData[indexPath.row];
            [_VcDelegate contactSelectViewCtrl:self selectedModel:tmpMo indexPath:self.indexPath];
        }
    }];
}
...
- (void)clickLeftButton:(UIButton *)sender {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_VcDelegate && [_VcDelegate respondsToSelector:@selector(contactSelectViewCtrlDismiss:)]) {
            [_VcDelegate contactSelectViewCtrlDismiss:self];
        }
    }];
}
...
- (SearchTopView *)searchView {
    if (!_searchView) {
        ...
        _searchView.searchTxtField.placeholder = @"请输入联系人/客户关键字";
    }
    return _searchView;
}
