...
#import "TrendsBusinessViewCtrl.h"
#import "TrendsBusinessCollectionCell.h"
#import "TrendsCreateBusinessViewCtrl.h"

@interface TrendsBusinessViewCtrl ()

@end

static NSString *identifier = @"TrendsBusinessCollectionCell";

@implementation TrendsBusinessViewCtrl

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"商机大全";
    self.rightBtn.hidden = NO;
    self.placeHoder = @"请输入客户、标题或内容关键字等";
...
//                                                      @"已关闭"]];
    ...
    [self updateSwitchTitles:@[@"商机来源", @"重要程度", @"活动日期"]];
    [self.collectionView registerClass:[TrendsBusinessCollectionCell class] forCellWithReuseIdentifier:identifier];
    [self refreshView];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    TrendsBusinessCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
...
- (void)switchView:(SwitchView *)switchView selectIndex:(NSInteger)index title:(NSString *)title switchState:(SwitchState)state {
    ...
    if (self.switchId == 2) {
        ...
        datepicker.wsCancelBlock = ^{
            [weakSelf.switchView updateTitle:@"活动日期" index:weakSelf.switchId switchState:SwitchStateNormal];
...
- (void)filterListView:(FilterListView *)filterListView didSelectIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.switchId == 0) {
        ...
        str = self.sortTag1 == 0 ? @"商机来源" : self.sortDic1.value;
    } else if (self.switchId == 1) {
        self.sortTag2 = indexPath.row;
        self.sortDic2 = self.arrSort2[self.sortTag2];
        str = self.sortTag2 == 0 ? @"重要程度" : self.sortDic2.value;
    }
    [self releaseFilterListView];
...
- (void)reloadDataWithParam {
    ...
    if (self.sortDic1.key.length > 0) {
        [rules addObject:@{@"field":@"resourceKey",
                           @"option":@"EQ",
