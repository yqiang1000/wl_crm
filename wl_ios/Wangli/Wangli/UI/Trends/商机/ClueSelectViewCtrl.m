...
@interface ClueSelectViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
{
...
@implementation ClueSelectViewCtrl
...
- (void)getMemberList:(NSInteger)page {
    ...
    if (self.searchKey.length > 0) [rules addObject:@{@"field":@"clueNumber-title-memberContactor",
                                                      @"option":@"LIKE_ANYWHERE",
                                                      @"values":@[STRING(self.searchKey)]}];
    if (rules.count > 0) [param setObject:rules forKey:@"rules"];
    [[JYUserApi sharedInstance] getClueItemPage:param success:^(id responseObject) {
        [Utils dismissHUD];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ClueMo *tmpMo = self.arrData[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_vcDelegate && [_vcDelegate respondsToSelector:@selector(clueSelectViewCtrl:didSelect:indexPath:)]) {
            ClueMo *tmpMo = self.arrData[indexPath.row];
            [_vcDelegate clueSelectViewCtrl:self didSelect:tmpMo indexPath:self.indexPath];
        }
    }];
}
...
- (void)clickLeftButton:(UIButton *)sender {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_vcDelegate && [_vcDelegate respondsToSelector:@selector(clueSelectViewCtrlDismiss:)]) {
            [_vcDelegate clueSelectViewCtrlDismiss:self];
        }
    }];
}
...
- (SearchTopView *)searchView {
    if (!_searchView) {
        ...
        _searchView.searchTxtField.placeholder = @"请输入线索关键字";
    }
    return _searchView;
}
