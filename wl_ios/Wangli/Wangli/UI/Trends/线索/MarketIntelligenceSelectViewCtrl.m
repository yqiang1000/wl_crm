//
//  MarketIntelligenceSelectViewCtrl.m
//  Wangli
//
...
@interface MarketIntelligenceSelectViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
{
...
@implementation MarketIntelligenceSelectViewCtrl
...
- (void)getMemberList:(NSInteger)page {
    ...
    if (self.searchKey.length > 0) [rules addObject:@{@"field":@"intelligenceTypeValue",
                                                      @"option":@"LIKE_ANYWHERE",
                                                      @"values":@[STRING(self.searchKey)]}];
    if (rules.count > 0) [param setObject:rules forKey:@"rules"];
    [[JYUserApi sharedInstance] getMarketIntelligenceItemPage:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self tableViewEndRefresh];
        NSError *error = nil;
        NSMutableArray *tmpArr = [MarketIntelligenceMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MarketIntelligenceMo *tmpMo = self.arrData[indexPath.row];
    cell.labText.text = tmpMo.content;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_vcDelegate && [_vcDelegate respondsToSelector:@selector(marketIntelligenceSelectViewCtrl:didSelect:indexPath:)]) {
            MarketIntelligenceMo *tmpMo = self.arrData[indexPath.row];
            [_vcDelegate marketIntelligenceSelectViewCtrl:self didSelect:tmpMo indexPath:self.indexPath];
        }
    }];
}
...
- (void)clickLeftButton:(UIButton *)sender {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_vcDelegate && [_vcDelegate respondsToSelector:@selector(marketIntelligenceSelectViewCtrlDismiss:)]) {
            [_vcDelegate marketIntelligenceSelectViewCtrlDismiss:self];
        }
    }];
}
...
- (SearchTopView *)searchView {
    if (!_searchView) {
        ...
        _searchView.searchTxtField.placeholder = @"请输入情报关键字";
    }
    return _searchView;
}
