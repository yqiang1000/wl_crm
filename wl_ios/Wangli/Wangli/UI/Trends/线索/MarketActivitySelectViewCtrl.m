//
//  MarketActivitySelectViewCtrl.m
//  Wangli
//
...
@interface MarketActivitySelectViewCtrl () <UITableViewDelegate, UITableViewDataSource, SearchTopViewDelegate>
{
...
@implementation MarketActivitySelectViewCtrl
...
- (void)getMemberList:(NSInteger)page {
    ...
    if (self.searchKey.length > 0) [rules addObject:@{@"field":@"title",
                                                      @"option":@"LIKE_ANYWHERE",
                                                      @"values":@[STRING(self.searchKey)]}];
    if (rules.count > 0) [param setObject:rules forKey:@"rules"];
    [[JYUserApi sharedInstance] getMarketActivityPage:param success:^(id responseObject) {
        [Utils dismissHUD];
        [self tableViewEndRefresh];
        NSError *error = nil;
        NSMutableArray *tmpArr = [MarketActivityMo arrayOfModelsFromDictionaries:responseObject[@"content"] error:&error];
        if (page == 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MarketActivityMo *tmpMo = self.arrData[indexPath.row];
    cell.labText.text = [NSString stringWithFormat:@"%@ | %@", tmpMo.title, tmpMo.content];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_vcDelegate && [_vcDelegate respondsToSelector:@selector(marketActivitySelectViewCtrl:didSelect:indexPath:)]) {
            MarketActivityMo *tmpMo = self.arrData[indexPath.row];
            [_vcDelegate marketActivitySelectViewCtrl:self didSelect:tmpMo indexPath:self.indexPath];
        }
    }];
}
...
- (void)clickLeftButton:(UIButton *)sender {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if (_vcDelegate && [_vcDelegate respondsToSelector:@selector(marketActivitySelectViewCtrlDismiss:)]) {
            [_vcDelegate marketActivitySelectViewCtrlDismiss:self];
        }
    }];
}
...
- (SearchTopView *)searchView {
    if (!_searchView) {
        ...
        _searchView.searchTxtField.placeholder = @"请输入活动关键字";
    }
    return _searchView;
}
