...
#import "TrendsClueViewCtrl.h"
#import "TrendsClueCollectionCell.h"
#import "TrendsCreateClueViewCtrl.h"

@interface TrendsClueViewCtrl ()

@end

static NSString *identifier = @"TrendsClueCollectionCell";

@implementation TrendsClueViewCtrl
...
//                                                      @"已关闭"]];
    ...
    [self updateSwitchTitles:@[@"线索来源", @"重要程度", @"创建日期"]];
    [self.collectionView registerClass:[TrendsClueCollectionCell class] forCellWithReuseIdentifier:identifier];
    [self refreshView];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    TrendsClueCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
...
- (void)switchView:(SwitchView *)switchView selectIndex:(NSInteger)index title:(NSString *)title switchState:(SwitchState)state {
    ...
    if (self.switchId == 2) {
        ...
        datepicker.wsCancelBlock = ^{
            [weakSelf.switchView updateTitle:@"创建日期" index:weakSelf.switchId switchState:SwitchStateNormal];
...
- (void)filterListView:(FilterListView *)filterListView didSelectIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.switchId == 0) {
        ...
        str = self.sortTag1 == 0 ? @"线索来源" : self.sortDic1.value;
    } else if (self.switchId == 1) {
