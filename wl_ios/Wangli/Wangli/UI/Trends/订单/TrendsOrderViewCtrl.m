...
#import "TrendsOrderViewCtrl.h"
#import "TrendsOrderCollectionCell.h"

@interface TrendsOrderViewCtrl ()

@end

static NSString *identifier = @"TrendsOrderCollectionCell";

@implementation TrendsOrderViewCtrl

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"订单清单";
    self.rightBtn.hidden = YES;
    self.placeHoder = @"请输入订单号或客户名称关键字";
...
//                                                      @"已取消"]];
    ...
    [self updateSwitchTitles:@[@"订单类型", @"销售组织", @"订单日期"]];
    [self.collectionView registerClass:[TrendsOrderCollectionCell class] forCellWithReuseIdentifier:identifier];
    [self refreshView];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    TrendsOrderCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
...
- (void)switchView:(SwitchView *)switchView selectIndex:(NSInteger)index title:(NSString *)title switchState:(SwitchState)state {
    ...
    if (self.switchId == 2) {
        ...
        datepicker.wsCancelBlock = ^{
            [weakSelf.switchView updateTitle:@"订单日期" index:weakSelf.switchId switchState:SwitchStateNormal];
...
- (void)filterListView:(FilterListView *)filterListView didSelectIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.switchId == 0) {
        ...
        str = self.sortTag1 == 0 ? @"订单类型" : self.sortDic1.value;
    } else if (self.switchId == 1) {
        self.sortTag2 = indexPath.row;
        self.sortDic2 = self.arrSort2[self.sortTag2];
        str = self.sortTag2 == 0 ? @"订单组织" : self.sortDic2.value;
    }
    [self releaseFilterListView];
