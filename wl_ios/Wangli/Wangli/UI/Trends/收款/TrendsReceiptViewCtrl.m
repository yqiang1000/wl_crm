...
#import "TrendsReceiptViewCtrl.h"
#import "TrendsReceiptCollectionCell.h"

@interface TrendsReceiptViewCtrl ()

@end

static NSString *identifier = @"TrendsReceiptCollectionCell";

@implementation TrendsReceiptViewCtrl

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"收款清单";
    self.rightBtn.hidden = YES;
    self.placeHoder = @"请输入收款单号、客户号或客户名称";
//    self.arrTitles = [NSMutableArray arrayWithArray:@[@"所有",
//                                                      @"广东公司",
//                                                      @"浙江公司",
//                                                      @"天津公司"]];
    [self updateSegmentTitles:self.arrTitles];
    [self updateSwitchTitles:@[@"收款方式", @"收款类型", @"收款日期"]];
    [self.collectionView registerClass:[TrendsReceiptCollectionCell class] forCellWithReuseIdentifier:identifier];
    [self refreshView];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    TrendsReceiptCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
...
- (void)switchView:(SwitchView *)switchView selectIndex:(NSInteger)index title:(NSString *)title switchState:(SwitchState)state {
    ...
    if (self.switchId == 2) {
        ...
        datepicker.wsCancelBlock = ^{
            [weakSelf.switchView updateTitle:@"收款日期" index:weakSelf.switchId switchState:SwitchStateNormal];
...
- (void)filterListView:(FilterListView *)filterListView didSelectIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.switchId == 0) {
        ...
        str = self.sortTag1 == 0 ? @"收款方式" : self.sortDic1.value;
    } else if (self.switchId == 1) {
        self.sortTag2 = indexPath.row;
        self.sortDic2 = self.arrSort2[self.sortTag2];
        str = self.sortTag2 == 0 ? @"收款类型" : self.sortDic2.value;
    }
    [self releaseFilterListView];
...
- (void)reloadDataWithParam {
    ...
    if (self.currentDic.key.length > 0) {
        [rules addObject:@{@"field":@"companyKey",
                           @"option":@"EQ",
    ...
    if (self.sortDic2.key.length > 0) {
        [rules addObject:@{@"field":@"type",
                           @"option":@"EQ",
                           @"values":@[STRING(self.sortDic2.key)]}];
    }
    if (self.selectDate.length > 0) {
        [rules addObject:@{@"field":@"receiptDate",
                           @"option":@"BTD",
